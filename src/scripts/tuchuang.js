...
dragF.addEventListener('change', inputFileHandler, false);
rsC.addEventListener('click', popCloseHandler, false);
...
function complete (src) {
	if (src) {
		...
		alert('上传需要一些问题。可能是您的新浪微博帐号已经登出了。请访问http://weibo.com/登录之后再过来。谢谢。');
	}
};
	...
	reader.onloadend = function(e) {		
		...
		xhr.onreadystatechange = function(){
			if (xhr.readyState === 4 && xhr.status === 200 ){
				...
				var rs         = JSON.parse(xhr.responseText.substring(splitIndex));
				var pid        = rs.data.pics.pic_1.pid;
				complete( pid2url(pid, 'large') );
			}
		};
